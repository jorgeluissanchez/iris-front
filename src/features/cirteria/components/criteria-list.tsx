    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
        ...
        <div className="w-full sm:flex-1">
          ...
          >
            {eventId ? (
              courses.length ? (
                courses.map((c) => <SelectItem key={c.id}>{c.code}</SelectItem>)
              ) : (
                <SelectItem key="no-courses" isDisabled>No courses</SelectItem>
              )
            ) : (
              <SelectItem key="select-event" isDisabled>Select event first</SelectItem>
            )}
          </Select>
        </div>
      </div>
      ...
      {meta && meta.totalPages > 1 && (
        <div className="flex justify-center mt-6">
          <Pagination total={meta.totalPages} page={page} onChange={handlePageChange} showControls />
        </div>
      )}
